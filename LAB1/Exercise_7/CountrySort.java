package aa224iu__assign1.Exercise_7;
...
public class CountrySort {
	...
	public static void main(String[] args) {
		...
		Comparator<String> longestName = (s1, s2) -> -(s1.length() - s2.length()); // Must be updated
		...
		Comparator<String> reverseAlphabetic = (s1, s2) -> -(s1.compareTo(s2)); // Must be updated
		Arrays.sort(COUNTRIES, reverseAlphabetic);
		System.out.println("\nCountries with reverse alphabetic names");
		for (int i = 0; i < 10; i++)
			System.out.println("\t" + COUNTRIES[i]);
		...
		Arrays.stream(COUNTRIES)
				.sorted((s1, s2) -> s2.replaceAll("[^aA]", "").length() - s1.replaceAll("[^aA]", "").length())
				.limit(10)
